...
*   **💻 暗黑模式**：
    *   **全局深色化**：将背景统一为 `#18181c`，卡片为 `#232326`，消除刺眼的白色色块。
    *   **控件美化**：重绘输入框、按钮、下拉菜单，去除多余阴影，增加扁平化硬朗质感。
    *   **自定义滚动条**：替换浏览器默认滚动条，使其与页面风格融为一体。
*   **📝 Markdown 深度适配**：
    *   完美适配作业详情页的攻略文本。
    *   优化代码块、引用、表格的显示效果，夜间阅读不伤眼。
*   **🧩 组件全覆盖**：
    *   修复了顶部导航栏、侧边抽屉 (Drawer)、弹窗 (Overlay) 在部分情况下反白的问题。
